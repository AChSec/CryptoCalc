...
def signature_main(ctx): 						
    ...
    insert_info(ctx.info_box, "Digital Signature with Message Recovery", digital_signature1)
    insert_info(ctx.info_box, "Digital Signature with Appended Hash", digital_signature2)
...
def create_signature(ctx):
    ...
    def run_elgamal():
        try:
            ...
            ctx.output_label.configure(text=f"Sent:\n\nMessage m: {m}\n\nSignature element s: {s}\n\nMessage identifier rho: {rho}\n\nAppended hash: \n{h}\n\n(Copy-friendly in info box)", font=("Arial", 14), justify="left")
    ...
    ctk.CTkLabel(inserts_rsa, text="RSA with Message Recovery", font=("Roboto", 14), anchor="center").grid(row = 0, column = 1, columnspan=3, pady=5)
    ...
    ctk.CTkButton(inserts_rsa, text="Calculate", command=run_rsa, anchor="center").grid (row= 3, column = 1, columnspan=3, padx=5, pady=5)
    ...
    ctk.CTkLabel(insert_frame, text="ElGamal with Appended Hash", font=("Roboto", 14), anchor="center").grid(row = 0, column = 1, columnspan=3, pady=5)
    ...
    ctk.CTkButton(insert_frame, text="Calculate", command=run_elgamal, anchor="center").grid (row= 6, column = 1, columnspan=3, padx=5, pady=5)

    ctk.CTkLabel(insert_frame, image=ctx.images["elgamal_sign"], text="").grid(row = 5, column=1, columnspan=3,padx=2, pady=2)

    insert_info(ctx.info_box, "ElGamal Appended Hash:", "Waiting for calculation...")
...
def verification(ctx):                  # page with verification options for both RSA and ElGamal
    ...
    ctk.CTkLabel(input_frame_rsa, text="RSA with Message Recovery", 
    ...
    ctk.CTkButton(input_frame_rsa, text="Verify", command=verify_rsa, 
    ...
    ctk.CTkLabel(input_frame_elgamal, text="ElGamal with Appended Hash", 
    ...
    ctk.CTkButton(input_frame_elgamal, text="Verify", command=verify_elgamal, 
                  width=80).grid (row= 4, column = 2, padx=5, pady=5)


    insert_info(ctx.info_box, "Use case:", "Digital Signature Algorithm (DSA)")
