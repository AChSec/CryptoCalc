...
def caesar(ctx):		
    def run():
        try:
            result = caesar_substitution(entry_text.get(), entry_key.get())
            ctx.output_label.configure(text=result + "\n\n" + caesar_output_info, font=("Roboto", 14))
        except Exception as e:
            ctx.output_label.configure(text=f"Error: {e}")
    
    entry_text = ctk.CTkEntry(ctx.content, placeholder_text="Enter text")
    entry_key = ctk.CTkEntry(ctx.content, placeholder_text="Key")
