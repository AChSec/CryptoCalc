...
def mac(ctx):
    def run():
        try:
            ...
            ctx.output_label.configure(text=f"{entry_a.get()} in ASCII-Code: {ascii_code}\n\The key {key} is prepended: {message}\n\nThe MAC is then computed from the result: {h}\n\nFor illustration purposes only. A MAC based on a block cipher (DES, AES) corresponds to the block size of the underlying algorithm", font=("Roboto", 14))
    ...
    entry_a = ctk.CTkEntry(mac_frame, placeholder_text="Plaintext", width=120)
    entry_b = ctk.CTkEntry(mac_frame, placeholder_text="Key", width=120)
    ...
    entry_k = ctk.CTkEntry(hmac_frame, placeholder_text="Key" , width=120)
    entry_m = ctk.CTkEntry(hmac_frame, placeholder_text="Plaintext", width=120)
