...
def rsa_main(ctx):
    ...
    insert_info(ctx.info_box, "Use case:", rsa_application)
...
def rsa_key(ctx):
    ...
    entry_q.pack(pady=5)
    entry_e = ctk.CTkEntry(ctx.content, placeholder_text="specify e:")
    entry_e.pack(pady=5)

    ctk.CTkButton(ctx.content, text="Calculate", command=run).pack(pady=10)

    insert_info(ctx.info_box, "Steps:", rsa_key_info)
...
def rsa_encrypt(ctx):
    ...
    entry_text = ctk.CTkEntry(input_frame, placeholder_text="Plaintext", width=170)
    ...
    ctk.CTkButton(input_frame, text="Calculate", command=run, width=110).grid(row=1, column=4, padx=5, pady=5)
...
def rsa_decrypt(ctx):
    ...
    def convert():
        ...
        ctx.output_label.configure(text=f"Plaintext = {text}", font=("Roboto", 14), justify="left")
    ...
    ctk.CTkButton(input_frame, text="Calculate", command=run, width=110).grid(row=0, column=4, padx=5, pady=(5, 10))
